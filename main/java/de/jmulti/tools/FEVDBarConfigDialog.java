...
public final class FEVDBarConfigDialog extends JDialog {
    ...
    JSCNArray getSelectedBarTypes() {
        ...
        for (int i = 0; i < selBarTypes.length; i++) {
            ...
            int shadingIndex = PlotControlDialog.SHADING_LIST
                    .lastIndexOf(shading);
    ...
    void setVariableNames(JSCSArray names) {
        ...
        if (!names.isEmpty()) {
            ...
            for (int i = 0; i < selShading.length; i++)
                // fill with solid as default
                selShading[i] = PlotControlDialog.GAUSS_SHADINGS[6];

            String[] selColor = new String[names.rows()];
            for (int i = 0; i < selColor.length; i++)
                // fill with solid as default
                selColor[i] = PlotControlDialog.GAUSS_COLORS[(i + 9)
                        % PlotControlDialog.GAUSS_COLORS.length];
