 ...
 */
public final class IRABootCIPercCall extends RPCall {
     ...
     */
    public IRABootCIPercCall(SymbolTable symbolTable, int periods,
    ...
    private String[] getCINames() {
        ...
        efronNameCI.append(coverageString + "% Efron Percentile CI (B="
                + numberOfRep + " h=" + periods);
        hallNameCI.append(coverageString + "% Hall Percentile CI (B="
                + numberOfRep + " h=" + periods);
    ...
    protected void runCode() {
        ...
        if (modelType == ModelTypes.SVAR) {
            String lastModel = gst.get(SVARConstants.lastModel_Def)
                    .getJSCString().string();
    ...
    private JSCNArray[] getRtnArgs() {
        JSCNArray[] retArgs = new JSCNArray[] { new JSCNArray("__rt_vec0"),
                ...
                new JSCNArray("__rt_vec7"), new JSCNArray("__rt_vec8"),
                new JSCNArray("__rt_vec9"), new JSCNArray("__rt_vec10"),
                (JSCNArray) IRABootCIStudCall.FE_BOOT.getInstance(),
                (JSCNArray) IRABootCIStudCall.ORTH_BOOT.getInstance() };
        return retArgs;
    }
    ...
    private void runSVECCode() {
        ...
        // Store in CI Manager
        // EFRON_ORTH_LOWER|EFRON_ORTH_UPPER
        JSCNArray[] gdEfron = new JSCNArray[] { retArgs[6], retArgs[7] };
        manager.addCI(efron_hall[0], modelType, gdEfron);

        // HALL_ORTH_LOWER|HALL_ORTH_UPPER
        JSCNArray[] gdHall = new JSCNArray[] { retArgs[8], retArgs[9] };
        manager.addCI(efron_hall[1], modelType, gdHall);
    ...
    private void runVARVECCode() {
        ...
        // Store in CI Manager
        // EFRON_FERR_LOWER|EFRON_FERR_UPPER|EFRON_ORTH_LOWER|EFRON_ORTH_UPPER
        JSCNArray[] gdEfron = new JSCNArray[] { retArgs[1], retArgs[2],
                retArgs[6], retArgs[7] };
        manager.addCI(efron_hall[0], modelType, gdEfron);

        // HALL_FERR_LOWER|HALL_FERR_UPPER|HALL_ORTH_LOWER|HALL_ORTH_UPPER
        JSCNArray[] gdHall = new JSCNArray[] { retArgs[3], retArgs[4],
                retArgs[8], retArgs[9] };
        manager.addCI(efron_hall[1], modelType, gdHall);
    ...
    private void setLocalResults(JSCNArray[] retArgs, boolean isSVARVEC) {
        ...
        if (local != null) {
            if (!isSVARVEC)
                local.get(IRABootCIStudCall.FE_BOOT).setJSCData(retArgs[11]);
