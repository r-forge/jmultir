 ...
 */
public final class JohCointTestCall extends RPCall {
    public static final JSCTypeDef JOH_RESULT = new JSCTypeDef("JOH_RESULT",
            JSCTypes.NARRAY,
            "Johansen test: LR statistic~critical values (1%,5%,10%)~p-values");
     ...
     *            levels lags (VAR)
     */
    public JohCointTestCall(JSCNArray endog, JSCSArray endogNames,
            JSCNArray dummies, JSCSArray dummyNames, boolean isSeasDum,
            int lagLength, int testState, TSDateRange range, int[] ecSelection,
            TSDate[] breakDates, boolean isTrendBreak) {
    ...
    protected void finalCode() {
        ...
        // user message
        if (testState == 3 && breakDates.length > 0)
            output
                    .append("Breaks are ignored for test version with trend orthogonal to EC term.\n");

        String[] namArray = new String[dummyNames.rows()];
        for (int i = 0; i < namArray.length; i++)
            namArray[i] = dummyNames.stringAt(i, 0);
        String resNames = UData.stringForArray(new JSCSArray("tmp",
                UStringArray.selRowsIf(namArray, ecSelection)));
        String unresNames = UData.stringForArray(new JSCSArray("tmp",
                UStringArray.delRowsIf(namArray, ecSelection)));
        boolean pValueComputed = true;
        if (Double.doubleToLongBits(joh_Result.doubleAt(0, 4)) == Double
                .doubleToLongBits(Double.NaN))
            pValueComputed = false;

        output.append(FArg.sprintf("%-25s %s \n", new FArg(
                "Johansen Trace Test for:").add(UData
                .stringForArray(endogNames))));
        if (namArray.length > 0) {
            output.append(FArg.sprintf("%-25s %s \n", new FArg(
                    "unrestricted dummies:").add(unresNames)));
            output.append(FArg.sprintf("%-25s %s \n", new FArg(
                    "restricted dummies:").add(resNames)));
        }
        ...
        if (testState == 1) {
            ...
            /*
             * if (!pValueComputed) ref = "reference: Johansen, S.
             * (1995),\n\"Likelihood-based Inference in Cointegrated VAR
             * models\",\nOxford University Press, Oxford, Table 15.2\n";
             */
        }
        if (testState == 2) {
             ...
             * if (!pValueComputed) ref = "reference: Johansen, S.
             * (1995),\n\"Likelihood-based Inference in Cointegrated VAR
             * models\",\nOxford University Press, Oxford, Table 15.4\n";
             */
        }
        if (testState == 3) {
             ...
             * if (!pValueComputed) ref = "reference: Johansen, S.
             * (1995),\n\"Likelihood-based Inference in Cointegrated VAR
             * models\",\nOxford University Press, Oxford, Table 15.3\n";
             */
        }
        if (!pValueComputed)
            ref = "critical values not available for this model\n";
        if (isSeasDum)
        ...
        output.append("-----------------------------------------------\n");
        if (pValueComputed)
            for (int i = 0; i < joh_Result.rows(); i++) {
                output.append(FArg.sprintf(
                        "%- 3i %- 8.2f %- 8.4f %- 8.2f %- 8.2f %- 8.2f",
                        new FArg(i).add(joh_Result.doubleAt(i, 0)).add(
                                joh_Result.doubleAt(i, 4)).add(
                                joh_Result.doubleAt(i, 1)).add(
                                joh_Result.doubleAt(i, 2)).add(
                                joh_Result.doubleAt(i, 3))));
                output.append("\n");
            }
        else
            for (int i = 0; i < joh_Result.rows(); i++) {
                output.append(FArg.sprintf("%- 3i %- 8.2f %-8s %-8s %-8s %-8s",
                        new FArg(i).add(joh_Result.doubleAt(i, 0)).add(
                                " " + joh_Result.doubleAt(i, 4)).add(
                                " " + joh_Result.doubleAt(i, 1)).add(
                                " " + joh_Result.doubleAt(i, 2)).add(
    ...
    protected void runCode() {
        ...
        if (getSymbolTable() != null)
            getSymbolTable().get(JOH_RESULT).clear();
        ...
        // get 1-based break date indices for GAUSS call
        int[] bIndices = new int[breakDates.length];
        for (int i = 0; i < bIndices.length; i++)
            // order is reversed to start with lowest index
            bIndices[i] = range
                    .indexForDate(breakDates[bIndices.length - 1 - i]) + 1;
        JSCNArray breakIndex = new JSCNArray("breakIndex", bIndices);

        if (!dummies.isEmpty()) {
            unresDum = new JSCNArray("unresDum", UMatrix.delColsIf(dummies
                    .doubleArray(), ecSelection));
            resDum = new JSCNArray("resDum", UMatrix.selColsIf(dummies
                    .doubleArray(), ecSelection));
        }
        ...
        engine().call(
                "johtest_cointjoh",
                new JSCData[] { endog, unresDum, seasDum, resDum,
                        new JSCInt("lags", lagLength),
                        new JSCInt("testState", testState), breakIndex,
                        new JSCInt("isTrendBreak", isTrendBreak) },
                new JSCData[] { joh_Result });

        if (getSymbolTable() != null)
            getSymbolTable().set(joh_Result);
     ...
     */
    public static JSCNArray createCondDummies(TSDate[] breakDates,
            TSDateRange origRange, int lags) {
     ...
     */
    public static int[] getECSelection(int[] ecSel, TSDate[] breakDates,
            int testState, boolean isTrendBreak, int levelsLags) {

        int userDum = ecSel.length;
        int numOfDetWithBreaks = userDum + breakDates.length * levelsLags
                + breakDates.length
                * ((isTrendBreak && testState == 2) ? 2 : 1);
        int[] ecSelection = new int[numOfDetWithBreaks];
        int k = 0;
        for (int val : ecSel)
            // keep orig restrictions
            ecSelection[k++] = val;

        // append restrictions to EC term for breaks according to spec
        if (breakDates.length > 0) {

            if (testState == 1) { // break in levels, restrict breaks to EC
                for (int i = 0; i < breakDates.length; i++)
                    ecSelection[ecSelection.length - i - 1] = 1;
            } else if (testState == 2) {
                // break in levels, restrict breaks to EC
                if (!isTrendBreak)
                    for (int i = 0; i < breakDates.length; i++)
                        ecSelection[ecSelection.length - i - 1] = 1;
                else
                    // break in trend, restrict trend breaks to EC but not level
                    // breaks
                    for (int i = 0; i < breakDates.length; i++) {
                        // restrict trend part
                        ecSelection[ecSelection.length - 2 * i - 1] = 1;
                        // do not restrict shift
     ...
     */
    public static String[] getBreakDumNames(TSDate[] breakDates, int testState,
            boolean isTrendBreak, int levelsLags) {
        //      assemble names for deterministics used for breaks
        String[] breakdumNames = new String[levelsLags * breakDates.length
                + breakDates.length
                * ((isTrendBreak && testState == 2) ? 2 : 1)];

        // dummies to condition on first p lags in each sample
        for (int i = 0, ii = 0; i < breakDates.length; i++)
            for (int j = 0; j < levelsLags; j++)
                breakdumNames[ii++] = "D["
                        + breakDates[breakDates.length - i - 1].addPeriods(j)
                        + "]";
        if (testState == 1) { // no trend break in any case
            for (int i = 0; i < breakDates.length; i++)
                breakdumNames[breakdumNames.length - 1 - i] = "S["
                        + breakDates[i] + "]";
        } else if (testState == 2) {
            if (!isTrendBreak) // no trend break
                for (int i = 0; i < breakDates.length; i++)
                    breakdumNames[breakdumNames.length - 1 - i] = "S["
                            + breakDates[i] + "]";
            else
                // trend break (shift and trend var alternating)
                for (int i = 0; i < breakDates.length; i++) {
                    breakdumNames[breakdumNames.length - 1 - 2 * i] = "t["
                            + breakDates[i] + "]";
                    breakdumNames[breakdumNames.length - 2 - 2 * i] = "S["
                            + breakDates[i] + "]";
                }
        }
     ...
     *  
     */
    public static JSCNArray createBreaks(TSDate[] breakDates,
            TSDateRange range, int testState, boolean isTrendBreak) {
        JSCNArray breaks = new JSCNArray("breaks");
        if (testState == 1) { // only levels break is used
            if (breakDates.length == 1) {
                double[] break1 = range.createShiftDum(new TSDateRange(
                        breakDates[0], range.upperBound()));
                ...
                JSCNArray dataBreak1 = new JSCNArray("break1", break1);
                breaks.appendCols(dataBreak1);
                double[] break2 = range.createShiftDum(new TSDateRange(
                        breakDates[1], breakDates[0].addPeriods(-1)));
                JSCNArray dataBreak2 = new JSCNArray("break1", break2);
                breaks.appendCols(dataBreak2);
            }
        } else if (testState == 2) { // trend and levels breaks possible
            if (breakDates.length == 1) {
                double[] break1 = range.createShiftDum(new TSDateRange(
                        breakDates[0], range.upperBound()));
                JSCNArray dataBreak1 = new JSCNArray("break1", break1);
                breaks.appendCols(dataBreak1);
                if (isTrendBreak) { // trend shift
                    double[] trendBreak1 = range
                            .createTrendShiftDum(new TSDateRange(breakDates[0],
                                    range.upperBound()));
                    JSCNArray dataTrendBreak1 = new JSCNArray("trendbreak1",
                            trendBreak1);
                ...
                breaks.appendCols(dataBreak2);
                if (isTrendBreak) {
                    double[] trendBreak2 = range
                            .createTrendShiftDum(new TSDateRange(breakDates[1],
                                    breakDates[0].addPeriods(-1)));
                    JSCNArray dataTrendBreak2 = new JSCNArray("trendbreak2",
                            trendBreak2);
