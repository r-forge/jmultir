...
public final class HegyTestCall extends RPCall {
    public static final JSCTypeDef HEGY_EST = new JSCTypeDef("HEGY_EST",
            JSCTypes.NARRAY, "Hegy test estimation results: est~t-values");

    public static final JSCTypeDef HEGY_RESIDS = new JSCTypeDef("HEGY_RESIDS",
            JSCTypes.NARRAY, "Hegy test: residuals from test regression");

    public static final JSCTypeDef HEGY_SIGMA = new JSCTypeDef("HEGY_SIGMA",
            JSCTypes.NUMBER, "Hegy test residual covariance");

    public static final JSCTypeDef HEGY_STAT = new JSCTypeDef("HEGY_STAT",
            JSCTypes.NARRAY,
            "Hegy test statistics:\nF34|F56|F78|F910|F1112|F1_12|F2_12 or F34|F234|F1234");

    public static final JSCTypeDef HEGY_CRITVAL = new JSCTypeDef(
            "HEGY_CRITVAL", JSCTypes.NARRAY,
     ...
     * </ul>
     */
    public HegyTestCall(JSCNArray y, JSCString yName, TSDateRange range,
    ...
    private void addRegressionResults(int testState) {
        ...
        if (period == 4)
            critNames = new String[] { "t(pi1)", "t(pi2)", "F34", "F234",
                    "F1234" };
        if (period == 12)
            critNames = new String[] { "t(pi1)", "t(pi2)", "F34", "F56", "F78",
                    "F910", "F1112", "F1-12", "F2-12" };
        ...
        case 2:
            ...
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "const").add(estimates[0][0]).add(estimates[0][1])));
            break;
        case 3:
            start = 2;
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "const").add(estimates[0][0]).add(estimates[0][1])));
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "trend").add(estimates[1][0]).add(estimates[1][1])));
            break;
        case 4:
            start = 5;
            for (int a = 0; a < period; a++)
                buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n",
                        new FArg("SD" + (a + 1)).add(estimates[a][0]).add(
                                estimates[a][1])));
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "trend").add(estimates[4][0]).add(estimates[4][1])));
            break;
        case 5:
            start = 4;
            for (int a = 0; a < period; a++)
        ...
        for (int a = start; a < estimates.length - 4; a++) {
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "AR" + (a - start)).add(estimates[a][0]).add(
                    estimates[a][1])));
            start = a;
        }
        double tpi[] = new double[2];
        tpi[0] = estimates[start + 1][1];
        tpi[1] = estimates[start + 2][1];

        for (int a = start + 1; a < estimates.length; a++)
            buffer.append(FArg.sprintf("%-13s %- 13.4f %- 13.4f\n", new FArg(
                    "pi" + (a - countStart + 1)).add(estimates[a][0]).add(
                    estimates[a][1])));
        ...
        buffer.append(FArg.sprintf("%-10s %-10s %-10s %-10s \n", new FArg(
                "statistic").add(" 1%").add(" 5%").add(" 10%")));
        for (int i = 0; i < critVals.length; i++)
            buffer.append(FArg.sprintf("%-10s %- 10.2f %- 10.2f %- 10.2f\n",
                    new FArg(critNames[i]).add(critVals[i][0]).add(
                            critVals[i][1]).add(critVals[i][2])));
        buffer.append("\nvalues of test statistics: \n");
        for (int i = 0; i < testStats.length + 2; i++)
            if (i < 2)
                buffer.append(FArg.sprintf("%-15s % .4f\n", new FArg(
                        critNames[i] + ":").add(tpi[i])));
            else
                buffer.append(FArg.sprintf("%-15s % .4f\n", new FArg(
                        critNames[i] + ":").add(testStats[i - 2][0])));
    ...
    protected void finalCode() {
        ...
        buffer.append(FArg.sprintf("%-25s %s \n", new FArg(
                "HEGY Test for series:").add(yName.string())));
        buffer.append(range.addPeriodsToStart(
    ...
    protected void runCode() {
        ...
        int period = range.lowerBound().subPeriodicity();
        if (y.rows() - (period + lags) < (2*period + lags + 2))
        ...
        String procName = "jm_hegy4";

        if (period == 4)
            engine().load("jm_hegy4.R", RStatLoadTypes.USERCODE);

        if (period == 12) {
            procName = "jm_hegy12";
            engine().load("jm_hegy12.R", RStatLoadTypes.USERCODE);
        }
        ...
        if (testState >= 4)
            seasDum = new JSCNArray("seasDum", range.createSeasDum(true, true));

        hegy_est.setJSCProperty(RArgProps.RLIST_KEY, "coeff");
        hegy_sigma.setJSCProperty(RArgProps.RLIST_KEY, "sigma");
        hegy_resids.setJSCProperty(RArgProps.RLIST_KEY, "resid");
        hegy_stats.setJSCProperty(RArgProps.RLIST_KEY, "hegy");
        hegy_crit.setJSCProperty(RArgProps.RLIST_KEY, "critVal");

        engine().call(
                procName,
                new JSCData[] { y, seasDum, new JSCInt("lags", lags),
                        new JSCInt("state", testState) },
                new JSCData[] { new JSCInt("T"), hegy_est, hegy_sigma,
                        hegy_resids, hegy_stats, hegy_crit });
