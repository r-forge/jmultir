...
public class ARIMAHanRissPCall extends RPCall {

    public final static JSCTypeDef ALL_LAGS_CRIT = new JSCTypeDef(
            "ALL_LAGS_CRIT", JSCTypes.NARRAY,
            "information criteria for all (pmax+1)^2 lag combinations: p~q~AIC~HQ~SC");

    public final static JSCTypeDef LAGS_OPT = new JSCTypeDef("LAGS_OPT",
            JSCTypes.NARRAY,
            "3 rows with optimal lags for AIC, HQ, SC, each row is p~q~AIC~HQ~SC");
    ...
    public ARIMAHanRissPCall(int h, int pqmax, int d, JSCNArray y,
            JSCNArray det, String yName, TSDateRange range) {
    ...
    protected void runCode() {
        ...
        engine().call(
                "hannan_rissanen_uni",
                new JSCData[] { y, det, new JSCInt("h", h),
                        new JSCInt("pmax", pqmax), new JSCInt("d", d) },
