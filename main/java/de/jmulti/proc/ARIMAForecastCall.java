...
public class ARIMAForecastCall extends RPCall {
    ...
    public final static JSCTypeDef FORECAST = new JSCTypeDef("FORECAST",
            JSCTypes.NARRAY, "3xh matrix: point forecast | lower CI |upper CI");

    public final static JSCTypeDef FORECAST_STDERR = new JSCTypeDef(
            "FORECAST_STDERR", JSCTypes.NARRAY,
            "h x 1 array of forecast std. errors");

    public ARIMAForecastCall(int p, int q, int d, JSCNArray y,
            JSCNArray resids, JSCNArray dets, JSCNArray b, String yName,
            double ciLevel, int horizon, TSDateRange range, TSDate startPlot,
            boolean isPlot, SymbolTable local) {
    ...
    protected void runCode() {
        ...
        engine().load("pgraph, tools, plot, arima, uni, var",
                GaussLoadTypes.LIB);

        JSCData[] inArgs = new JSCData[] { b, y, new JSCInt("p", p),
                new JSCInt("d", d), new JSCInt("q", q), dets, resids,
        ...
        // y should be shown from t=0, which is the beginning of the
        // modeling
        // period
        ...
        engine().call(
                "plot_forecast",
                new JSCData[] { yOrig, forecast, new JSCNumber("T1", T1),
                        new JSCNumber("T2", T2), new JSCNArray("selForec", 1),
                        new JSCSArray("ny", new String[] { yName }),
                        new JSCString("tit", title),
                        new JSCNumber("beg", startPlot.doubleValue()),
    ...
    protected void finalCode() {
        TSDateRange r = new TSDateRange(range.upperBound().addPeriods(1), range
                .upperBound().addPeriods(horizon));
        ...
        for (int i = 0; i < forecast.cols(); i++) 
            output.append(FArg.sprintf(
                    "%-14s %- 20.4f %- 20.4f %- 20.4f %- 20.4f\n", times[i],
                    forecast.doubleAt(1, i), forecast.doubleAt(0, i), forecast
                            .doubleAt(2, i), forecast_stderr.doubleAt(i, 0)));
