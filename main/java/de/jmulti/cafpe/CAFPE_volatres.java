...
/**
 * Panel that binds components of residual analysis for volatility estimation
 * together.
 * 
 * @author <a href="mailto:mk@mk-home.de">Markus Kraetzig</a>
 */

final class CAFPE_volatres extends JPanel {
    private static final Logger log = Logger.getLogger(CAFPE_volatres.class);
    ...
    private CAFPE_diagTests getCAFPE_diagTests() {
        if (ivjCAFPE_diagTests == null) {
            try {
                ...
                ivjCAFPE_diagTests.setVolatRes(true);
                ivjCAFPE_diagTests
                        .setResiduals(CAFPE_constants.RESID_EST_VOLAT);
                ivjCAFPE_diagTests.setXsadj(CAFPE_constants.XSADJ_VOLAT);
    ...
    private de.jmulti.tools.KernDensPanel getKernelDensity() {
        if (ivjKernelDensity == null) {
            try {
                ...
                // user code begin {1}
                ivjKernelDensity
                        .setResids(CAFPE_constants.RESID_EST_VOLAT.name);
                ivjKernelDensity
                        .setNameResids(CAFPE_constants.NAME_RESIDS_VOLAT.name);
                ivjKernelDensity.setDRange(CAFPE_constants.T1_volat.name);
    ...
    private de.jmulti.tools.PlotAddResPanel getPlotAddResids() {
        if (ivjPlotAddResids == null) {
            try {
                ...
                ivjPlotAddResids
                        .setNameResids(CAFPE_constants.NAME_RESIDS_VOLAT.name);
                ivjPlotAddResids.setDRange(CAFPE_constants.T1_volat.name);
    ...
    private de.jmulti.tools.SpectrumPanel getSpectrum() {
        if (ivjSpectrum == null) {
            try {
                ...
                // user code begin {1}
                ivjSpectrum.setResids(CAFPE_constants.RESID_EST_VOLAT.name);
                ivjSpectrum
                        .setNameResids(CAFPE_constants.NAME_RESIDS_VOLAT.name);
                ivjSpectrum.setDRange(CAFPE_constants.T1_volat.name);
    ...
    private void initialize() {
        ...
        TitledBorder title = new TitledBorder(new BevelBorder(
                BevelBorder.LOWERED),
                "Check Residuals of Volatility Estimation", TitledBorder.RIGHT,
                TitledBorder.TOP);
        setBorder(title);
        // user code end
    }
