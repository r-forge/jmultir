...
final class STR_Estimate extends ModelPanel {
    ...
    class IvjEventHandler implements java.awt.event.ActionListener,
        ...
        public void actionPerformed(java.awt.event.ActionEvent e) {
            if (e.getSource() == STR_Estimate.this.getExecute())
                connEtoC1();
            if (e.getSource() == STR_Estimate.this.getRestrictions())
                connEtoC3();
        };

        public void itemStateChanged(java.awt.event.ItemEvent e) {
            if (e.getSource() == STR_Estimate.this.getLSTR2())
                connEtoC2(e);
        };
    };
    ...
    /**
     * connEtoC1: (Execute.action. --> EstimateStar.execute_ActionEvents()V)
     */
    ...
    /**
     * connEtoC2: (LSTR2.item.itemStateChanged(java.awt.event.ItemEvent) -->
     * EstimateStr.lSTR2_ItemStateChanged()V)
     * 
     * @param arg1
    ...
    /**
     * connEtoC3: (Restrictions.action. -->
     * EstimateStr.restrictions_ActionEvents()V)
     */
    /* WARNING: THIS METHOD WILL BE REGENERATED. */
    ...
    private void execute_ActionEvents() {
        ...
        int loops = 0;
        Double loopD = UString.parseToNumber(getMaxLoops().getSelectedItem()
                .toString());
        if (loopD == null)
            loops = 0;
        else
            loops = loopD.intValue();
        ...
        // start values
        JSCNArray startGammaC1C2 = new JSCNArray("gammaC1C2", new double[] {
                getGamma().getNumber(), getC1().getNumber(),
                getC2().getNumber() });

        // this is used by test for parameter constancy.
        global().get(STR_Constants.STR_CONST_EST).setJSCData(selConst);
        global().get(STR_Constants.STR_PHIRES_EST).setJSCData(selPhi0);

        // Invoke the procedure call.
        PCall job = new STREstimationCall(global().get(STR_Constants.STR_Y)
                ...
                selPhi0, selThetMinPhi, global().get(
                        STR_Constants.STR_thet0Names).getJSCSArray(), global()
                        .get(STR_Constants.STR_allRes).getJSCNArray(),
                startGammaC1C2, loops, k);
        ...
        job.addPCallListener(new PCallAdapter() {
            public void success() {
                global().get(STR_Constants.STR_TRANSINDEX).setJSCData(
                        new JSCInt("toSet", selTransIndex));

                String transName = global().get(STR_Constants.STR_transNames)
                        .getJSCSArray().stringAt(selTransIndex, 0);

                global().get(STR_Constants.STR_TRANSNAME).setJSCData(
                        new JSCString("toSet", transName));
            }
        });
        job.execute();
        // if succesfull set transition variable
    ...
    private javax.swing.JPanel getControlPanel() {
        if (ivjControlPanel == null) {
            try {
                ...
                constraintsRestrictions.insets = new java.awt.Insets(0, 20, 0,
                        0);
                ...
                TitledBorder title = new TitledBorder(new BevelBorder(
                        BevelBorder.LOWERED), "Estimate STR Model",
    ...
    /**
     * Return the Restrictions property value.
     * 
     * @return javax.swing.JButton
     */
    /* WARNING: THIS METHOD WILL BE REGENERATED. */
    private javax.swing.JButton getRestrictions() {
        if (ivjRestrictions == null) {
            try {
                ...
                ivjRestrictions
                        .setPreferredSize(new java.awt.Dimension(120, 26));
                // user code begin {1}
    ...
    private STR_RestrictChooser getResChooser() {
        if (strRestrictionChooser == null) {
            strRestrictionChooser = new STR_RestrictChooser(TopFrameReference
    ...
    private void restrictions_ActionEvents() {
        getResChooser().setLocationRelativeTo(
