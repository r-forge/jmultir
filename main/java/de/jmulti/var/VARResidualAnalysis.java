...
/**
 * Panel that binds the VAR residual analysis together.
 * 
 * @author <a href="mailto:mk@mk-home.de">Markus Kraetzig</a>
 */
public final class VARResidualAnalysis extends ModelPanel {
    private static final Logger log = Logger
            .getLogger(VARResidualAnalysis.class);
    ...
    private CorrPanel getCorrelation() {
        if (ivjCorrelation == null) {
            try {
                ...
                // user code begin {1}
                ivjCorrelation.setModelType(ModelTypes.VAR);
                // user code end
    ...
    private de.jmulti.tools.KernDensPanel getKernelDensity() {
        if (ivjKernelDensity == null) {
            try {
                ...
                // user code begin {1}
                ivjKernelDensity.setResids(VARConstants.u_hat_Def.name);
                ivjKernelDensity.setNameResids(VARConstants.Nu_Def.name);
                ivjKernelDensity.setDRange(VARConstants.T1_Def.name);
    ...
    private MultDiagTests getMultivariateDiagnosticTests() {
        if (ivjMultivariateDiagnosticTests == null) {
            try {
                ...
                // user code begin {1}
                ivjMultivariateDiagnosticTests.setModelType(ModelTypes.VAR);
                // user code end
            } catch (Throwable ivjExc) {
                // user code begin {2}
                // user code end
    ...
    private de.jmulti.tools.PlotAddResPanel getPlotAddResids() {
        if (ivjPlotAddResids == null) {
            try {
                ...
                // user code begin {1}
                ivjPlotAddResids.setResids(VARConstants.u_hat_Def.name);
                ivjPlotAddResids.setNameResids(VARConstants.Nu_Def.name);
                ivjPlotAddResids.setDRange(VARConstants.T1_Def.name);
    ...
    private de.jmulti.tools.SpectrumPanel getSpectrum() {
        if (ivjSpectrum == null) {
            try {
                ...
                // user code begin {1}
                ivjSpectrum.setResids(VARConstants.u_hat_Def.name);
                ivjSpectrum.setNameResids(VARConstants.Nu_Def.name);
                ivjSpectrum.setDRange(VARConstants.T1_Def.name);
    ...
    private void initialize() {
        try {
            ...
            TitledBorder title = new TitledBorder(new BevelBorder(
                    BevelBorder.LOWERED), "VAR Residual Analysis",
