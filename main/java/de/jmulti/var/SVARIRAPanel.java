...
/**
 * This panel binds the components of the SVAR impulse response analysis
 ...
 * data objects. A table change listener resets the computed IR if necessary.
 * 
 * @author <a href="mailto:mk@mk-home.de">Markus Kraetzig</a>
 */
public final class SVARIRAPanel extends ModelPanel {
    private static final Logger log = Logger.getLogger(SVARIRAPanel.class);
    ...
    /**
     * SVARIRAPanel constructor comment.
     */
    public SVARIRAPanel() {
        super();
        initialize();
    ...
    private IRAComputePanel getIRAComputePanel() {
        if (ivjIRAComputePanel == null) {
            try {
                ...
                ivjIRAComputePanel.setModelType(ModelTypes.SVAR);
                // user code begin {1}
                // user code end
    ...
    private IRADisplayPanel getIRADisplayPanel() {
        if (ivjIRADisplayPanel == null) {
            try {
                ...
                ivjIRADisplayPanel.setModelType(ModelTypes.SVAR);
                // user code begin {1}
                // user code end
            } catch (Throwable ivjExc) {
                // user code begin {2}
    ...
    private void initialize() {
        ...
        // for resetting IRA if model was reestimated, assuming
        // svar_A0_Def is cleared before
        global().get(SVARConstants.svar_A0_Def).addSymbolListener(
                new SymbolListener() {
